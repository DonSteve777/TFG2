...
//data sizes , (percents of max N)
	...
	try{
		csvFile.open("tiemposTLE.csv", std::ofstream::trunc);	//If the file is opened for output operations and it already existed, its previous content is deleted and replaced by the new one.
	...
	for (int i = 0; i < OBSERVATIONS; i++) {
		...
		if (pid == -1) {
			...
			const char* const argx[] = {"tle.exe", nullptr};
			if (execvp("./tle.exe", argx ) == -1)
				exit(EXIT_FAILURE);
		} else {
